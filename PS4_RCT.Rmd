...
**Answer:** 
This table is determining whether there are any biases in the groups (control and experimental) at baseline. It mirrors Table 1 in the paper.
...
**Code:**
```{r, Q5, results ='asis'}
r4 <- felm(data=Round2_f, cash ~ migrant + migrant_new)
r5 <- felm(data=Round2_f, credit ~ migrant + migrant_new)
r6 <- felm(data=Round2_f, info ~ migrant + migrant_new)
r7 <- felm(data=Round2_f, control ~ migrant + migrant_new)
...
```{r}
#Model
r8<-felm(average_exp2~cash+credit+info|0|0|village, data=Round2_f_sub)
summary(r8)
```
...
cluster_se <- as.vector(summary(r8,cluster = c("village"))$coefficients[,"Cluster s.e."])
...
```{r}
r9<-felm(average_exp2~cash+credit+info, data=Round2_f_sub)
summary(r9)
```


```{r, Q9, results ='asis'}
stargazer(r9, type="text", header=FALSE, title="Effects of Migration on Consumption Among Remaining Household Members", 
                     style="qje", 
                     se = NULL,
                     covariate.labels = c("Cash", "Credit", "Info"),
                     dep.var.labels.include = FALSE,
